...
*********************************************/
  ...
  shopManager.prototype.init = function (_json) {
    ...
    for (var i in _json.shop) {
      ret.stash.items[i] = this.createObject(
        _json.shop[i].unlocked,
        _json.shop[i].quantity
      );
    }
  ...
  shopManager.prototype.saveStash = function (_stash) {
    ...
    for (var i in _stash.stash.items) {
      if (
        _stash.stash.items[i].quantity <= 0 &&
        _stash.stash.items[i].unlocked == 0
      ) {
  ...
  shopManager.prototype.loadStash = function (_shop, _stash) {
    ...
    for (var i in _shopInited.stash.items) {
      ...
      _shopInited.stash.items[i].quantity = Math.min(
        _shopInited.stash.items[i].quantity,
        baseShop.shop[i].quantityMax
      );
    }
