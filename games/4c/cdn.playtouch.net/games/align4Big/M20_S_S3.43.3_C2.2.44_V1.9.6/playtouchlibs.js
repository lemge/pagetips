...
*********************************************/
  ...
  baseConverter.IntToBase = function (value, base) {
    ...
    return base > 9 && value < 10
      ? parseInt(value.toString(base)) * (neg ? -1 : 1)
      : (neg ? "-" : "") + value.toString(base);
  };
...
*********************************************/
  ...
  var Seed = function () {
    this.availableEngine = {
      anonymous: {
        ...
        checkSeed: function (val) {
          //int only, lower than 233280
      ...
      alea: {
        ...
        customInit: function () {
          this.m = new (function () {
            ...
            return function (r) {
              for (
                var f, t, s, u = 0, e = 0.02519603282416938;
                u < r.length;
                u++
              )
                (s = r.charCodeAt(u)),
          ...
          this.c = this.m(" ");
          this.y = 0;
          (this.a -= this.m(this.initialValue)),
            (this.b -= this.m(this.initialValue)),
        ...
        getNextValue: function () {
          var y = this.value * 2.3283064365386963e-10 + this.a * 2091639,
            a = this.b,
            b = this.c;
          var value = y | 0;
          var c = y - value;
  ...
  Seed.prototype.checkEngine = function (engine) {
    if (
      ...
      typeof this.availableEngine[engine] !== "object"
    )
      return typeof seedsField !== "undefined" &&
        typeof seedsField.DEFAULT_PRNG_ENGINE !== "undefined"
        ? seedsField.DEFAULT_PRNG_ENGINE
        : "anonymous";
    return engine;
  };
  ...
  Seed.prototype.init = function (seedInitialValue, label, engine) {
    ...
    this.initialValue = this.availableEngine[this.engine].checkSeed.apply(
      this,
      arguments
    );
    ...
    if (this.label !== "")
      window.eventToFire.fireEvent("c2:seed_" + this.label + ":init");
  ...
  Seed.prototype.grow = function (step) {
    ...
    if (this.label !== "")
      window.eventToFire.fireEvent("c2:seed_" + this.label + ":grow", step);
    window.eventToFire.fireEvent("c2:seed:grow", this, step);
    return this;
  };

  Seed.prototype.getNextValue = function () {
    return this.availableEngine[this.engine].getNextValue.apply(
      this,
      arguments
    );
    // return (this.value * 9301 + 49297) % 233280;
  ...
  Seed.prototype.random = function (max, min, grow) {
    ...
    if (this.label !== "")
      window.eventToFire.fireEvent(
        "c2:seed_" + this.label + ":random",
        ret,
        max,
        min
      );
  ...
  seedsField.getSeedNextValue = function (seedName, defaultRet) {
    ...
    if (typeof seedName !== "string" || seedName === "")
      seedName = this.DEFAULT_SEED_NAME;
    if (typeof this.seeds[seedName] === "undefined") return defaultRet;
    return this.seeds[seedName].getNextValue();
  };

  seedsField.getPrediction = function (seedName, max, min, defaultRet) {
    return typeof seedName === "undefined" ||
      typeof this.seeds[seedName] === "undefined"
      ? typeof defaultRet !== "undefined"
        ? defaultRet
        : false
      : new Seed()
          .init(
            this.seeds[seedName].initialValue,
            undefined,
            this.seeds[seedName].engine
          )
          .grow(this.seeds[seedName].step)
          .random(max, min, defaultRet);
  };
  ...
  seedsField.getSeedRandomAt = function (
    seedInitialValue,
    engine,
    step,
    max,
    ...
    return new Seed()
      .init(seedInitialValue, undefined, engine)
      .grow(step)
      .random(max, min);
  };

  seedsField.plant(
    seedsField.DEFAULT_SEED_NAME,
    undefined,
    seedsField.DEFAULT_PRNG_ENGINE
  );
...
*********************************************/
  ...
  versionChecker.versionStrToNum = function (str_vers) {
    ...
    return (
      versionSplitted[0] * 1000000 +
      versionSplitted[1] * 1000 +
      versionSplitted[2] * 1
    );
  };
...
*********************************************/
  ...
  arrayWaitForFunction.prototype.waitForFunction = function (
    timeToWait,
    callback,
    param,
    persistentLevel
    ...
    param =
      Number(param) !== Number(param) || param === " " ? param : Number(param);
    this.array.push({
      time: timeToWait,
      callback: callback,
      param: param,
      id: this.id,
      persistentLevel:
        typeof persistentLevel === "undefined" ? 0 : persistentLevel,
    });
    return this.id++;
  };
  ...
  arrayWaitForFunction.prototype.getTimeUntilEndOf = function (
    id,
    defaultTime
  ) {
...
*********************************************/
  ...
  nfsm.prototype.next = function (state) {
    ...
    var beforeExitActualState = c2_callFunction(
      "beforeExit_" + this.currentState,
      [
        this.currentState,
        state,
        { current: this.currentState, nextState: state },
      ],
      PLIBS_TAG + ":" + this.__proto__.tag
    );
    var beforeEnterNextState = c2_callFunction(
      "beforeEnter_" + state,
      [
        this.currentState,
        state,
        {
          ...
          nextState: state,
          beforeExitResult: beforeExitActualState,
        },
      ],
      PLIBS_TAG + ":" + this.__proto__.tag
    );
    window.eventToFire.fireEvent(
      "c2:nfsm:nextRequest",
      {
        current: this.currentState,
        current: this.currentState,
        beforeExit: beforeExitActualState,
        beforeEnter: beforeEnterNextState,
      },
      PLIBS_TAG + ":" + this.__proto__.tag
    );
    if (beforeExitActualState == 0 && beforeEnterNextState == 0) {
      c2_callFunction(
        "onExit_" + this.currentState,
        [
          this.currentState,
          state,
          { current: this.currentState, next: this.nextState },
        ],
        PLIBS_TAG + ":" + this.__proto__.tag
      );
      ...
      this.nextState = "";
      window.eventToFire.fireEvent("c2:nfsm:nextSucceed", {
        previous: this.previousState,
        current: this.currentState,
      });
      c2_callFunction(
        "onEnter_" + this.currentState,
        [
          this.currentState,
          previousState,
          { current: this.currentState, previous: previousState },
        ],
        PLIBS_TAG + ":" + this.__proto__.tag
      );
    } else {
      window.eventToFire.fireEvent("c2:nfsm:nextFailed", {
        current: this.currentState,
        rejected: state,
        next: this.nextState,
      });
      c2_callFunction(
        "onFailExit_" + this.currentState,
        [
          this.currentState,
          state,
          beforeExitActualState,
          beforeEnterNextState,
          {
            current: this.currentState,
            nextState: state,
            beforeExitActualState: beforeExitActualState,
            beforeEnterNextState: beforeEnterNextState,
          },
        ],
        PLIBS_TAG + ":" + this.__proto__.tag
      );
...
*********************************************/
  ...
  timeNotifier.prototype.getCompressedStr = function (obj) {
    ...
    for (var anAC in obj) {
      ...
      if (obj[anAC]["lastAck"] > -1)
        toRet[anAC]["a"] = baseConverter.IntToBase(
          Math.max(0, obj[anAC]["timeStart"] - obj[anAC]["lastAck"]),
          36
        );
      if (obj[anAC]["time"] !== 86400)
        toRet[anAC]["t"] = baseConverter.IntToBase(obj[anAC]["time"], 36);
      if (
        obj[anAC]["timeEnd"] - obj[anAC]["time"] * 1000 !==
        obj[anAC]["timeStart"]
      )
        toRet[anAC]["d"] = baseConverter.IntToBase(
          Math.max(0, obj[anAC]["timeEnd"] - obj[anAC]["timeStart"]),
          36
        );
      if (obj[anAC]["message"] !== "") toRet[anAC]["m"] = obj[anAC]["message"];
      if (obj[anAC]["state"] !== this.state.ACTIVE)
        toRet[anAC]["e"] = obj[anAC]["state"] === this.state.UNACTIVE ? 0 : 1;
    }
    return JSON.stringify(toRet);
  };
  ...
  timeNotifier.prototype.getUnCompressedObj = function (objStr) {
    ...
    for (var anAC in obj) {
      if (!obj.hasOwnProperty(anAC)) continue;
      if (typeof obj[anAC]["name"] !== "undefined") {
        toRet[anAC] = Object.assign({}, obj[anAC]);
        continue;
      }
      toRet[anAC] = { name: anAC };
      toRet[anAC]["time"] =
        typeof obj[anAC]["t"] === "undefined"
          ? 86400
          : baseConverter.baseToInt(obj[anAC]["t"], 36);
      toRet[anAC]["state"] =
        typeof obj[anAC]["e"] === "undefined"
          ? this.state.ACTIVE
          : obj[anAC]["e"] == 0
          ? this.state.UNACTIVE
          : this.state.PENDING;
      toRet[anAC]["message"] =
        typeof obj[anAC]["m"] === "undefined" ? "" : obj[anAC]["m"];
      toRet[anAC]["timeStart"] = baseConverter.baseToInt(obj[anAC]["s"], 36);
      toRet[anAC]["lastAck"] =
        typeof obj[anAC]["a"] === "undefined"
          ? -1
          : toRet[anAC]["timeStart"] -
            baseConverter.baseToInt(obj[anAC]["a"], 36);
      toRet[anAC]["timeEnd"] =
        typeof obj[anAC]["d"] === "undefined"
          ? toRet[anAC]["timeStart"] + toRet[anAC]["time"] * 1000
          : toRet[anAC]["timeStart"] +
            baseConverter.baseToInt(obj[anAC]["d"], 36);
      toRet[anAC]["count"] =
        typeof obj[anAC]["c"] === "undefined" ? 0 : obj[anAC]["c"];
    }
    return toRet;
  };

  timeNotifier.prototype.load = function () {
    this.arrayNotif = this.getUnCompressedObj(
      c2_callFunction(
        "readCustomData",
        [this.saveKey, "{}"],
        PLIBS_TAG + ":" + this.__proto__.tag
  ...
  timeNotifier.prototype.save = function () {
    ...
    c2_callFunction(
      "writeCustomData",
      [this.saveKey, this.getCompressedStr(this.arrayNotif), ""],
      PLIBS_TAG + ":" + this.__proto__.tag
    );
    window.eventToFire.fireEvent("c2:timeNotifier:save");
  };

  timeNotifier.prototype.set = function (
    eventName,
    eventTime,
    eventMessage,
    notifyDevice,
    ...
    if (typeof this.arrayNotif[eventName] === "undefined") {
      this.arrayNotif[eventName] = { count: 0, lastAck: -1 };
    }
    ...
    if (notifyDevice) {
      window.eventToFire.fireEvent(
        "c2:timeNotifier:onSetNotification",
        this.arrayNotif[eventName],
        notifMessage,
        notifTitle
      );
    }
  ...
  timeNotifier.prototype.ack = function (eventName) {
    ...
    if (this.arrayNotif[eventName].state != this.state.PENDING) {
      return;
    }
  ...
  timeNotifier.prototype.getValue = function (eventName, key, defaultRet) {
    ...
    if (
      typeof this.arrayNotif[eventName] === "undefined" ||
      typeof key === "undefined" ||
      typeof this.arrayNotif[eventName][key] === "undefined"
    )
      return defaultRet;
  ...
  timeNotifier.prototype.checkNotifByName = function (eventName) {
    ...
    if (this.arrayNotif[eventName].state == this.state.UNACTIVE) {
      return;
    }
    if (Date.now() < this.arrayNotif[eventName].timeEnd) {
      return;
    }

    this.arrayNotif[eventName].state = this.state.PENDING;
    c2_callFunction(
      "timeNotifier_onNotification",
      [
        eventName,
        this.arrayNotif[eventName].eventMessage,
        JSON.stringify(this.arrayNotif[eventName]),
      ],
      PLIBS_TAG + ":" + this.__proto__.tag
    );
    this.save();
    window.eventToFire.fireEvent(
      "c2:timeNotifier:onNotification",
      this.arrayNotif[eventName]
    );
    window.eventToFire.fireEvent(
      "c2:timeNotifier:" + eventName,
      this.arrayNotif[eventName]
    );
  };
  ...
  timeNotifier.prototype.getSecondBySpecificTime = function (
    hours,
    minutes,
    seconds
  ) {
...
*********************************************/
  ...
  leaderboard.prototype.init = function () {
    window.eventToFire.registerEvent(
      "leaderboard:onSubmitScoreSucceeded",
      function (result) {
        c2_callFunction(
          "leaderboard_onSubmitScoreSucceeded",
          [],
          PLIBS_TAG + ":" + this.__proto__.tag
        );
      }
    ...
    window.eventToFire.registerEvent(
      "leaderboard:onSubmitScoreFailed",
      function (result) {
        c2_callFunction(
          "leaderboard_onSubmitScoreFailed",
          [],
          PLIBS_TAG + ":" + this.__proto__.tag
        );
      }
    ...
    var that = this;
    window.eventToFire.registerEvent(
      "leaderboard:ICanDisplayLeaderboard",
      function (result) {
        if (typeof result !== "boolean") result = true;
        that.capableToDisplayLeaderboard = result;
      }
    );
  ...
  leaderboard.prototype.submitScore = function (leaderboard, score) {
    ...
    if (typeof leaderboard !== "string" || typeof score === "undefined")
      return 0;
    window.eventToFire.fireEvent(
      "c2:leaderboard:submitScore",
      leaderboard,
      score
    );
    return 1;
  ...
  leaderboard.prototype.showLeaderboard = function (leaderboard, canFallback) {
    ...
    window.eventToFire.fireEvent(
      "c2:leaderboard:showLeaderboard",
      leaderboard,
      canFallback
    );
    return 1;
...
*********************************************/
  ...
  achievement.prototype.getCompressedStr = function (obj) {
    ...
    for (var anAC in obj) {
      ...
      toRet[anAC] = {};
      if (
        obj[anAC]["currentStep"] > 0 &&
        obj[anAC]["achievement_state"] !== this.state.UNLOCKED
      )
        toRet[anAC]["s"] = baseConverter.IntToBase(
          obj[anAC]["currentStep"],
          36
        );
      if (!obj[anAC]["seen"]) toRet[anAC]["e"] = 0;
      if (obj[anAC]["achievement_type"] === this.type.INCREMENTAL)
        toRet[anAC]["m"] = baseConverter.IntToBase(
          obj[anAC]["achievement_steps"],
          36
        );
      if (obj[anAC]["achievement_state"] !== this.state.REVEALED)
        toRet[anAC]["a"] =
          obj[anAC]["achievement_state"] === this.state.HIDDEN ? 0 : 1;
    }
    return JSON.stringify(toRet);
  };
  ...
  achievement.prototype.getUnCompressedObj = function (objStr) {
    ...
    for (var anAC in obj) {
      if (!obj.hasOwnProperty(anAC)) continue;
      if (typeof obj[anAC]["achievement_code"] !== "undefined") {
        toRet[anAC] = Object.assign({}, obj[anAC]);
        continue;
      }
      toRet[anAC] = { achievement_code: anAC };
      toRet[anAC]["seen"] = typeof obj[anAC]["e"] === "undefined";
      toRet[anAC]["achievement_type"] =
        typeof obj[anAC]["m"] === "undefined"
          ? this.type.STANDARD
          : this.type.INCREMENTAL;
      toRet[anAC]["achievement_steps"] =
        typeof obj[anAC]["m"] === "undefined"
          ? 1
          : baseConverter.baseToInt(obj[anAC]["m"], 36);
      toRet[anAC]["achievement_state"] =
        typeof obj[anAC]["a"] === "undefined"
          ? this.state.REVEALED
          : obj[anAC]["a"] > 0
          ? this.state.UNLOCKED
          : this.state.HIDDEN;
      toRet[anAC]["currentStep"] =
        typeof obj[anAC]["s"] !== "undefined"
          ? baseConverter.baseToInt(obj[anAC]["s"], 36)
          : toRet[anAC]["achievement_state"] === this.state.UNLOCKED
          ? toRet[anAC]["achievement_steps"]
          : 0;
    }
    return toRet;
  ...
  achievement.prototype.load = function () {
    this.arrayAchievement = this.getUnCompressedObj(
      c2_callFunction(
        "readCustomData",
        [this.saveKey, "{}"],
        PLIBS_TAG + ":" + this.__proto__.tag
  ...
  achievement.prototype.save = function () {
    ...
    c2_callFunction(
      "writeCustomData",
      [this.saveKey, this.getCompressedStr(this.arrayAchievement), ""],
      PLIBS_TAG + ":" + this.__proto__.tag
    );
  ...
  achievement.prototype.register = function (
    achievement_code,
    achievement_step,
    achievement_obj
  ) {
    if (typeof this.arrayAchievement[achievement_code] !== "undefined") {
      return window.eventToFire.fireEvent(
        "c2:achievement:register",
        this.arrayAchievement[achievement_code]
      );
    }
    var achievement_obj = achievement_obj || {};
    if (typeof achievement_obj == "string") {
      achievement_obj = JSON.parse(achievement_obj);
    }
    this.arrayAchievement[achievement_code] = new achievementObj().create(
      achievement_code,
      achievement_step,
      achievement_obj["achievement_state"] || this.state.REVEALED,
      achievement_obj["achievement_type"] ||
        (achievement_step == 1 ? this.type.STANDARD : this.type.INCREMENTAL)
    );
    this.save();
    window.eventToFire.fireEvent(
      "c2:achievement:register",
  ...
  achievement.prototype.unlock = function (achievement_code) {
    if (!this.arrayAchievement[achievement_code]) {
      return;
    }
    var acObj = this.arrayAchievement[achievement_code];
    if (acObj.achievement_type != this.type.STANDARD) {
      return;
    }
  ...
  achievement.prototype.increment = function (
    achievement_code,
    stepsToIncrement
  ) {
    if (!this.arrayAchievement[achievement_code]) {
    ...
    var acObj = this.arrayAchievement[achievement_code];
    if (acObj.achievement_type != this.type.INCREMENTAL) {
      return;
    }
    if (acObj.currentStep >= acObj.achievement_steps) {
      return;
  ...
  achievement.prototype.get = function (achievement_code, key) {
    if (typeof achievement_code != "undefined" && typeof key != "undefined") {
      ...
      if (typeof this.arrayAchievement[achievement_code][key] == "undefined") {
        return -1;
      }
  ...
  achievement.prototype.reveal = function (achievement_code) {
    ...
    this.arrayAchievement[achievement_code].achievement_state =
      this.state.REVEALED;
    this.save();
    window.eventToFire.fireEvent(
      "c2:achievement:reveal",
      this.arrayAchievement[achievement_code]
    );
  };
