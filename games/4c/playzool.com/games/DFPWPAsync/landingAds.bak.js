...
function showFuncName(obj) {
    console.log("正在运行方法：" + obj.callee.name);
}
...
function requestAds(isInter) {
    ...
    if (isInter) {
        ...
        adsRequest.adTagUrl = 'https://pubads.g.doubleclick.net/gampad/ads?sz=400x300|640x480&iu=/14206847/Playzool_Mid-roll&ciu_szs&impl=s&gdfp_req=1&env=vp&output=vast&unviewed_position_start=1&overlay=0&description_url=https%3A%2F%2Fplayzool.com%2F%3Fshow%3Dsplash%26game%3Dalign4Big_PZOOL&cust_params=uua%3DMozilla%2F5.0+%28X11%3B+Linux+x86_64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F107.0.0.0+Safari%2F537.36+Edg%2F107.0.1418.28%26userIP%3D5.188.230.99%26fpartner%3Dplaytouch%26_p%3Dp_playtouch%26gameCode%3Dalign4Big%26rewarded%3Dfalse%26c_playtouch%3Dc_align4Big';
    } else {
        ...
        adsRequest.adTagUrl = 'https://pubads.g.doubleclick.net/gampad/ads?sz=400x300|640x480&iu=/14206847/test&ciu_szs&impl=s&gdfp_req=1&env=vp&output=vast&unviewed_position_start=1&overlay=0&description_url=https%3A%2F%2Fplayzool.com%2F%3Fshow%3Dsplash%26game%3Dalign4Big_PZOOL&cust_params=uua%3DMozilla%2F5.0+%28X11%3B+Linux+x86_64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F107.0.0.0+Safari%2F537.36+Edg%2F107.0.1418.28%26userIP%3D5.188.230.99%26fpartner%3Dplaytouch%26_p%3Dp_playtouch%26gameCode%3Dalign4Big%26rewarded%3Dfalse%26c_playtouch%3Dc_align4Big';
    }
    ...
    if (_debug) {
        for (var index in google.ima.AdEvent.Type) {
            adsLoader.addEventListener(
                ...
                function () {
                    console.log("adsLoader.AdEvent :::", google.ima.AdEvent.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
        for (var index in google.ima.AdErrorEvent.Type) {
            adsLoader.addEventListener(
                ...
                function () {
                    console.log("adsLoader.AdErrorEvent :::", google.ima.AdErrorEvent.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
        for (var index in google.ima.AdsManagerLoadedEvent.Type) {
            adsLoader.addEventListener(
                ...
                function () {
                    console.log("adsLoader.AdsManagerLoadedEvent :::", google.ima.AdsManagerLoadedEvent.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
        for (var index in google.ima.CustomContentLoadedEvent.Type) {
            adsLoader.addEventListener(
                ...
                function () {
                    console.log("adsLoader.CustomContentLoadedEvent :::", google.ima.CustomContentLoadedEvent.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
        for (var index in google.ima.AdError.Type) {
            adsLoader.addEventListener(
                ...
                function () {
                    console.log("adsLoader.AdError :::", google.ima.AdError.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
    }
...
function onAdEvent(adEvent) {
    console.log("正在运行", arguments.callee.name)
    if (_debug) {
        console.log(actualTypeOfPub, adEvent.type);
    }
...
function clickOnButonAds() {
    ...
    if (inIframe() && document.referrer.indexOf('gogy.com') !== -1)
		window.top.location.href = window.location.href.split("?")[0] + "?fromPartner=gogy&utm_source=gogy&utm_medium=frameBreaker&utm_campaign=gogy.com";
...
var eventOnClick = setTimeout(function () {
    console.log("正在运行111", arguments.callee.name)
...
hideAll();
console.log("nopreroll", typeof (nopreroll))
    ...
    if (_debug) {
        for (var index in google.ima.AdEvent.Type) {
            adsManager.addEventListener(
                google.ima.AdEvent.Type[index],
                function () {
                    console.log("adsManager.AdEvent :::", google.ima.AdEvent.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
        for (var index in google.ima.AdErrorEvent.Type) {
            adsManager.addEventListener(
                google.ima.AdErrorEvent.Type[index],
                function () {
                    console.log("adsManager.AdErrorEvent :::", google.ima.AdErrorEvent.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
        for (var index in google.ima.AdsManagerLoadedEvent.Type) {
            adsManager.addEventListener(
                google.ima.AdsManagerLoadedEvent.Type[index],
                function () {
                    console.log("adsManager.AdsManagerLoadedEvent :::", google.ima.AdsManagerLoadedEvent.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
        for (var index in google.ima.CustomContentLoadedEvent.Type) {
            adsManager.addEventListener(
                google.ima.CustomContentLoadedEvent.Type[index],
                function () {
                    console.log("adsManager.CustomContentLoadedEvent :::", google.ima.CustomContentLoadedEvent.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
        for (var index in google.ima.AdError.Type) {
            adsManager.addEventListener(
                google.ima.AdError.Type[index],
                function () {
                    console.log("adsManager.AdError :::", google.ima.AdError.Type[this.index], arguments);
                }.bind({index: index})
            );
        }
    }
        ...
        if (actualTypeOfPub == "preroll") {
            ...
            $(window).resize(function () {
                ...
                if (currentAd.contentType.indexOf("video") == -1) {
                    if ($(window).width() < adWidth + 40) {
                        return;
                    }
                    if ($(window).height() < adHeight + 75) {
                        return;
                    }
                }
                var windowsSize = getSizeWindow();
...
function onAdError(adErrorEvent) {
    console.log("正在运行", arguments.callee.name)
    if (_debug) {
        var lastAdError = adErrorEvent;
        console.log("onAdError", adErrorEvent.getError());
    }
    if (typeof (fireEvent) != 'undefined') {
        fireEvent("IMA_" + actualTypeOfPub + '_' + google.ima.AdErrorEvent.Type.AD_ERROR)
    }
    ;
    if (typeof (fireEvent) != 'undefined') {
        fireEvent(actualTypeOfPub + '_error')
    }
    ;
    if (typeof (fireEvent) != 'undefined') {
        fireEvent(actualTypeOfPub + '_end')
    }
    ;
    isDisplayingAd = false;
    ...
    if (actualTypeOfPub == "preroll") {
        ...
        if (window.location !== window.parent.location) $('#c2canvasdiv').one("click", function () {
            launchPubInter("postponedPreroll");
        });
...
function launchPubInter() {
    ...
    if (typeof (adsManager) != "undefined") {
        ...
        CDBetweenInterTimer = setTimeout(function () {
            canFireAnInter = true;
        }, CDBetweenInter);
        adsManager.start();
    } else {
...
eventToFire.registerEvent("c2LayoutChange", function (state, name, force) {
    setTimeout(onC2LayoutChange, 1, state, name, force)
});
