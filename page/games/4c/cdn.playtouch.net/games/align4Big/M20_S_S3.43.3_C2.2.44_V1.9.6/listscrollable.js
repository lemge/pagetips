(function () {
  ...
  waitingListManager.prototype.createList = function (
    listName,
    orientation,
    startPos,
    height,
    ...
    this.waitList[listName] = new WaitingList().create(
      this,
      listName,
      orientation,
      startPos,
  ...
  waitingListManager.prototype.setStartPos = function (listName, startPos) {
    ...
    this.waitList[listName].setStartPos(
      typeof startPos != "undefined" ? startPos : 0
    );
  };
  waitingListManager.prototype.setOrientation = function (
    listName,
    orientation
  ) {
    if (!this.waitList[listName]) {
    ...
    this.waitList[listName].setOrientation(
      typeof orientation != "undefined" &&
        (orientation === ORIENTATION.HORIZONTAL ||
          orientation === ORIENTATION.VERTICAL)
        ? orientation
  ...
  waitingListManager.prototype.setOffset = function (listName, offset) {
    ...
    this.waitList[listName].setOffset(
      typeof offset != "undefined" ? offset : 0
    );
  };
  waitingListManager.prototype.setHeight = function (listName, height) {
    ...
    this.waitList[listName].setHeight(
      typeof height != "undefined" ? height : 0
    );
  };
  waitingListManager.prototype.setOffsetStart = function (
    listName,
    offsetStart
  ) {
    if (!this.waitList[listName]) {
    ...
    this.waitList[listName].setOffsetStart(
      typeof offsetStart != "undefined" ? offsetStart : 0
    );
  };
  waitingListManager.prototype.setRoundScroll = function (
    listName,
    roundScroll,
    smoothActive,
    smoothSpeed
    ...
    this.waitList[listName].setRoundScroll(
      !!(typeof roundScroll != "undefined" ? roundScroll : false),
      !!(typeof smoothActive != "undefined" ? smoothActive : false),
      typeof smoothSpeed != "undefined" ? smoothSpeed : 10
    );
  };
  waitingListManager.prototype.setClampObject = function (
    listName,
    clampMin,
    clampMax,
    offsetBetweenClamp
    ...
    this.waitList[listName].setClampObject(
      typeof clampMin != "undefined" ? clampMin : null,
      typeof clampMax != "undefined" ? clampMax : null,
      typeof offsetBetweenClamp != "undefined" ? offsetBetweenClamp : 0
    );
  };
  waitingListManager.prototype.setCanMoveByUid = function (
    listName,
    uid,
    canMove
  ) {
  ...
  waitingListManager.prototype.setVisibleObject = function (
    listName,
    uid,
    visible
  ) {
  ...
  waitingListManager.prototype.setTemporaryPosDrop = function (
    listName,
    temporaryPos
  ) {
    if (!this.waitList[listName]) {
  ...
  waitingListManager.prototype.setActiveCallbackByName = function (
    listName,
    callbackName,
    active
  ) {
  ...
  waitingListManager.prototype.getPercentObjectOnList = function (
    listName,
    uid
  ) {
    if (!this.waitList[listName]) {
  ...
  waitingListManager.prototype.getRandomObjectVisibleOnList = function (
    listName
  ) {
    if (!this.waitList[listName]) {
      return false;
  ...
  waitingListManager.prototype.movePercent = function (
    listName,
    scrollPercentAdd
  ) {
    if (!this.waitList[listName]) {
  ...
  waitingListManager.prototype.addImpulsion = function (
    listName,
    inertialVelocity
  ) {
    if (!this.waitList[listName]) {
  ...
  WaitingList.prototype.create = function (
    waitingListManager,
    listName,
    orientation,
    startPos,
  ...
  WaitingList.prototype.setRoundScroll = function (
    roundScroll,
    smoothActive,
    smoothSpeed
  ) {
  ...
  WaitingList.prototype.setClampObject = function (
    clampMin,
    clampMax,
    offsetBetweenClamp
  ) {
  ...
  WaitingList.prototype.setActiveCallbackByName = function (
    callbackName,
    active
  ) {
  ...
  WaitingList.prototype.getRandomObjectOnList = function () {
    if (this.listOfObject.length <= 0) {
      return -1;
    }
    return this.listOfObject[
      Math.floor(Math.random() * this.listOfObject.length)
    ].uid;
  };
  WaitingList.prototype.getRandomObjectVisibleOnList = function () {
    ...
    return listVisible.length > 0
      ? listVisible[Math.floor(Math.random() * listVisible.length)].uid
      : -1;
  };
  ...
  WaitingList.prototype.endMove = function () {
    if (this.roundScroll) {
      if (this.roundScroll_activeSmooth) {
        if (this.listOfObjectVisible > 1) {
          ...
          if (this.actualDirection < 0) {
            newPos = this.getScrollByPercent(
              floorTo(
                this.getScrollPercent(),
                100 / (this.listOfObjectVisible - 1)
              )
            );
      ...
      if (this.activeCallback.listEndMoved)
        c2_callFunction(C2FUNC.listEndMoved, [this.listName]), "scrollList";
    }
  };
  ...
  WaitingList.prototype.getScrollPercent = function () {
    return Math.abs(
      ((this.offsetScroll - this.offset / 2) * 100) /
        (this.maxScroll - this.offset / 2)
    );
  };
  WaitingList.prototype.getScrollByPercent = function (percent) {
    return (
      ((this.maxScroll - this.offset / 2) * percent) / 100 + this.offset / 2
    );
  };
  ...
  WaitingList.prototype.calculMaxScroll = function () {
    this.maxScroll =
      this.height + this.offset / 2 - this.offset * this.listOfObjectVisible;
  };
  // ------------------------------------------------------------------------------
  ...
  WaitingList.prototype.addToList = function (uid, object, newPos) {
    ...
    newPos =
      typeof newPos === "undefined"
        ? this.listOfObject.length
        : clamp(newPos, 0, this.listOfObject.length);
  ...
  WaitingList.prototype.removeToList = function (uid) {
    ...
    if (typeof posCaseToRemove === "undefined") {
      console.log("NEW CASE TO REMOVE FROM THE LIST IS NOT FOUND");
      return;
    }
    for (var i = 0; i < this.listOfObject.length; i++) {
  ...
  WaitingList.prototype.update = function (dt) {
    ...
    if (this.inertialVelocity != 0) {
      ...
      this.inertialVelocity =
        this.inertialVelocity * (1 - this.INERTIA_ACCELERATION * dt);
      var saveScroll = this.offsetScroll;
      if (
        Math.abs(this.inertialVelocity) < this.INERTIA_THRESHOLD ||
        saveScroll != this.offsetScroll
      ) {
        this.inertialVelocity = 0;
        this.endMove();
      }
    ...
    if (this.roundScroll_smoothScrollToMove != 0) {
      var toRemove =
        this.roundScroll_smoothScrollToMove * this.roundScroll_smoothSpeed * dt;
      this.addScroll(toRemove);
      this.roundScroll_smoothScrollToMove -= toRemove;
      if (
        Math.abs(this.roundScroll_smoothScrollToMove) <
        this.roundScroll_THRESHOLD
      ) {
        this.roundScroll_smoothScrollToMove = 0;
        this.endMoveRoundScroll();
      }
  ...
  WaitingList.prototype.updateObjVisibility = function () {
    var idToShow = 0;
    this.listOfObject.sort(function (a, b) {
      return a.pos - b.pos;
    });

    for (var i = 0; i < this.listOfObject.length; i++) {
      this.listOfObject[i].posToShow = this.listOfObject[i].visible
        ? idToShow++
        : -1;
    }
  ...
  WaitingListObj.prototype.removeIt = function () {
    if (this.list.activeCallback.objListRemoved)
      c2_callFunction(C2FUNC.objListRemoved, [this.uid], "scrollList");
  };
  ...
  WaitingListObj.prototype.update = function () {
    ...
    var newX =
      this.list.orientation == ORIENTATION.VERTICAL
        ? this.list.startPos
        : this.getPosObject();
    var newY =
      this.list.orientation == ORIENTATION.HORIZONTAL
        ? this.list.startPos
        : this.getPosObject();

    //clamp
    if (
      this.list.clampMinPosObject != null &&
      this.list.clampMaxPosObject != null
    ) {
      var clampMin =
        this.list.clampMinPosObject +
        this.posToShow * this.list.offsetBetweenClamp;
      var clampMax =
        this.list.clampMaxPosObject -
        (this.list.listOfObjectVisible - 1 - this.posToShow) *
          this.list.offsetBetweenClamp;

      if (this.list.orientation == ORIENTATION.HORIZONTAL)
        newX = clamp(newX, clampMin, clampMax);
      if (this.list.orientation == ORIENTATION.VERTICAL)
        newY = clamp(newY, clampMin, clampMax);
    }
    //move if necessary
    if (newX != this.object.x || newY != this.object.y) {
      ...
      if (this.list.activeCallback.objListMoved)
        c2_callFunction(C2FUNC.objListMoved, [this.object.uid], "scrollList");
    }
  ...
  WaitingListObj.prototype.getPosObject = function () {
    ...
    return (
      this.list.offsetStart +
      this.list.offsetScroll +
      (this.posToShow +
        (this.list.temporaryPos != -1 &&
        this.posToShow >= this.list.temporaryPos
          ? 1
          : 0)) *
        this.list.offset
    );
  ...
  WaitingListObj.prototype.getPercentOnList = function () {
    ...
    if (
      ...
      var clampMax =
        ...
        this.list.offsetStart;
      if (this.list.orientation == ORIENTATION.HORIZONTAL)
        pos = clamp(pos, clampMin, clampMax);
      if (this.list.orientation == ORIENTATION.VERTICAL)
        pos = clamp(pos, clampMin, clampMax);
