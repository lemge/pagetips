...
var autoCatalog2_rm = {
    args: {struType: 'htmlele', dataS: 'body', style: 'catalog', leaveClose: false},
    catalogData: [],
    init: function (args) {
        // 初始化
        this.catalogData = []
...
var autoCatalog2 = {
    ...
    setCataLogDom: function () {
        ...
        aclbox.classList.add('acl-box')
        aclbox.classList.add('acl-box-hidden')
        var swb = document.createElement('a')
        swb.classList.add('acl-swb')
        swb.innerHTML="☰目录"
        ...
        function setListUl(catalogData) {
            ...
            ulist.classList.add('acl-ul')
            for (let i = 0; i < catalogData.length; i++) {
                let nowli = catalogData[i]
                let tmpLi = document.createElement('li')
                tmpLi.classList.add('acl-li')
                ...
                if (nowli.c.length > 0) {
                    ...
                    tmpA.onclick=function (o){
                        ...
                        if(tmpUl.dataset.Stru==='open'){
                            now_a.querySelector(".acl-sws").innerText = '▶'
                            tmpUl.classList.add('aclist-hidden');
                            tmpUl.dataset.Stru='close'
                        }else{
                            now_a.querySelector(".acl-sws").innerText = '▼'
                            tmpUl.classList.remove('aclist-hidden');
                            tmpUl.dataset.Stru='open'
                        }
                        return false;
                    }
    ...
    sw_btn_Func: function (o) {
        console.log('sw_btn_Func')
        let aclBox=o.target.parentNode;
        if(aclBox.dataset.Status==='open'){
            aclBox.classList.remove("acl-box-show")
            aclBox.classList.add("acl-box-hidden")
            aclBox.dataset.Status='close'
        }else{
            aclBox.classList.remove("acl-box-hidden")
            aclBox.classList.add("acl-box-show")
            aclBox.dataset.Status='open'
        }
    },
    leave_Func: function (o) {
        console.log('leave_func')
        let aclBox=o.target.parentNode;
